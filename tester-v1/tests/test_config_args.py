...
class TestLoadServerConfigArgs(unittest.TestCase):
    def _write_server_yaml(self, tmp: str, body: str) -> Path:
    ...
    def test_block_scalar_parses_args(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            yaml_path = self._write_server_yaml(
                tmp,
                "args: |\n"
                "  --host 127.0.0.1\n"
                "  --port 9090\n"
    ...
    def test_missing_args_is_empty(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            yaml_path = self._write_server_yaml(tmp, "")
            cfg = load_server_config(yaml_path)
            self.assertEqual(cfg.args, [])

    def test_empty_block_scalar_is_empty(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            yaml_path = self._write_server_yaml(tmp, "args: |\n")
            cfg = load_server_config(yaml_path)
            self.assertEqual(cfg.args, [])
