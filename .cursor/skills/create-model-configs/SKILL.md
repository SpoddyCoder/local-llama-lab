...
Default: probe stdout for footprint, ctx-probe, and hello-world-baseline (YAML from `configs/reference/`), then a six-line summary (throughput plus VRAM/context; no JSON). Calibration assumes 512 MiB GPU VRAM before the model loads; use `--margin-mib` if that baseline differs. Add `--save-result` when probe JSON under `results/{model_slug}/{variant}/` is needed on disk (gitignored); calibration also writes `results/{model_slug}/calibration-sessions/{session_id}.json`. Copy summary lines into the repo README when documenting the model.
